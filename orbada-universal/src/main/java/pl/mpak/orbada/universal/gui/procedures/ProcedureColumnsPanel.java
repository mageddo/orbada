 ...
 * @author  akaluza
 */
public class ProcedureColumnsPanel extends javax.swing.JPanel implements ITabObjectInfo {
  ...
  private void init() {
    ...
    try {
      ...
      tableColumns.addColumn(new QueryTableColumn("COLUMN_NAME", stringManager.getString("argument-name"), 150, new QueryTableCellRenderer(java.awt.Font.BOLD)));
      tableColumns.addColumn(new QueryTableColumn("TYPE_NAME", stringManager.getString("argument-type"), 150, new QueryTableCellRenderer(SwingUtil.Color.GREEN)));
      tableColumns.addColumn(new QueryTableColumn("LENGTH", stringManager.getString("length"), 60));
      tableColumns.addColumn(new QueryTableColumn("PRECISION", stringManager.getString("precision"), 60));
      tableColumns.addColumn(new QueryTableColumn("NULLABLE", stringManager.getString("null-q"), 30));
  ...
  public void refresh() {
    try {
      ...
      tableColumns.getQuery().setResultSet(getDatabase().getMetaData().getProcedureColumns(currentCatalogName, currentSchemaName, currentProcedureName, null));
  ...
  public void refresh(String catalogName, String schemaName, String objectName) {
    if (!StringUtil.equals(catalogName, currentCatalogName) || !StringUtil.equals(schemaName, currentSchemaName) || !currentProcedureName.equals(objectName) || requestRefresh) {
      currentCatalogName = catalogName;
      currentSchemaName = schemaName;
      currentProcedureName = objectName;
      if (isVisible()) {
        refresh();
      }
      else {
  ...
  public String getTitle() {
    return stringManager.getString("ProcedureColumnsPanel-title");
  }
