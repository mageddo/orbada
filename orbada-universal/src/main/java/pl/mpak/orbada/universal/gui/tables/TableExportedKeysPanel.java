...
public class TableExportedKeysPanel extends javax.swing.JPanel implements ITabObjectInfo {
  ...
  private void init() {
    ...
    try {
      tableExportedKeys.addColumn(new QueryTableColumn("FK_NAME", stringManager.getString("key-name"), 150));
      tableExportedKeys.addColumn(new QueryTableColumn("FKTABLE_SCHEM", stringManager.getString("ref-schema"), 150));
      tableExportedKeys.addColumn(new QueryTableColumn("FKTABLE_NAME", stringManager.getString("ref-table"), 150, new QueryTableCellRenderer(java.awt.Font.BOLD)));
      tableExportedKeys.addColumn(new QueryTableColumn("FKCOLUMN_NAME", stringManager.getString("ref-column"), 150));
    } catch (Exception ex) {
      ExceptionUtil.processException(ex);
    }
  }
  ...
  public void refresh() {
    try {
      ...
      tableExportedKeys.getQuery().setResultSet(getDatabase().getMetaData().getExportedKeys(currentCatalogName, currentSchemaName, currentTableName));
