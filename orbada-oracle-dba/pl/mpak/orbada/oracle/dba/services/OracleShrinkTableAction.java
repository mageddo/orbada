...
import pl.mpak.orbada.oracle.dba.gui.wizards.ShrinkTableWizard;
 ...
 * @author akaluza
 */
public class OracleShrinkTableAction extends ComponentActionProvider {
        ...
        if (getComponent() instanceof ViewTable) {
          ...
          if (vt.getSelectedRow() >= 0) {
            try {
              ...
              SqlCodeWizardDialog.show(new ShrinkTableWizard(getDatabase(), schemaName, tableName), true);
            } catch (Exception ex) {
  ...
  public boolean isForComponent(Database database, String actionType) {
    ...
    setTooltip(stringManager.getString("OracleShrinkTableAction-hint"));
    setActionCommandKey("OracleShrinkTableAction");
  ...
  public String getDescription() {
    return stringManager.getString("OracleShrinkTableAction-description");
  }
