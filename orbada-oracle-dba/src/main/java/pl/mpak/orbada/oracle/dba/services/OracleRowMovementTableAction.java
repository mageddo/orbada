...
import pl.mpak.orbada.oracle.dba.gui.wizards.RowMovementTableWizard;
 ...
 * @author akaluza
 */
public class OracleRowMovementTableAction extends ComponentActionProvider {
        ...
        if (getComponent() instanceof ViewTable) {
          ...
          if (vt.getSelectedRow() >= 0) {
            try {
              ...
              SqlCodeWizardDialog.show(new RowMovementTableWizard(getDatabase(), schemaName, tableName), true);
            } catch (Exception ex) {
  ...
  public boolean isForComponent(Database database, String actionType) {
    ...
    setTooltip(stringManager.getString("OracleRowMovementTableAction-hint"));
  ...
  public String getDescription() {
    return stringManager.getString("OracleRowMovementTableAction-description");
  }
