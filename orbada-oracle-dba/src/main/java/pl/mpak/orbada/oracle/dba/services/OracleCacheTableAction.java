...
import pl.mpak.orbada.oracle.gui.wizards.table.CacheTableWizard;
 ...
 * @author akaluza
 */
public class OracleCacheTableAction extends ComponentActionProvider {
        ...
        if (getComponent() instanceof ViewTable) {
          ...
          if (vt.getSelectedRow() >= 0) {
            try {
              ...
              SqlCodeWizardDialog.show(new CacheTableWizard(getDatabase(), schemaName, tableName), true);
            } catch (Exception ex) {
  ...
  public boolean isForComponent(Database database, String actionType) {
    ...
    setTooltip(stringManager.getString("OracleCacheTableAction-hint"));
    setActionCommandKey("OracleCacheTableAction");
  ...
  public String getDescription() {
    return stringManager.getString("OracleCacheTableAction-description");
  }
