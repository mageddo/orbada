 ...
 * @author akaluza
 */
public class JdbcDbFunctionInfo extends DbObjectContainer {
  ...
  public JdbcDbFunctionInfo(String name,JdbcDbFunctionListInfo owner) {
    super(name, owner);
    put(new DefaultDbObjectContainer<JdbcDbFunctionParameterInfo>("PARAMETERS", this) {
  ...
  public void refresh() throws Exception {
    ...
    try {
      ...
      try {
        ...
        query.setResultSet(getDatabase().getMetaData().getFunctionColumns(getCatalog(), schema == null ? null : schema.getName(), getObjectName(), null));
        while (!query.eof()) {
      ...
      privileges.clear();
      try {
        query.setResultSet(getDatabase().getMetaData().getTablePrivileges(getCatalog(), getSchema().getName(), getObjectName()));
        while (!query.eof()) {
