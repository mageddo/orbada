...
public class TriggerSourcePanel extends javax.swing.JPanel implements ITabObjectInfo {
  ...
  private void setEditorText(final String objectName, final String text) {
    java.awt.EventQueue.invokeLater(new Runnable() {
      public void run() {
        syntaxSource.setDatabaseObject(currentSchemaName, "TRIGGER", objectName, text);
  ...
  private void init() {
    ...
    new ComponentActionsAction(getDatabase(), syntaxSource.getEditorArea(), buttonActions, menuActions, "oracle-trigger-source-actions");
  }
  ...
  public void refresh() {
    ...
    new SourceCreator(getDatabase(), syntaxSource).getSource(currentSchemaName, "TRIGGER", currentObjectName);
...
private void cmStoreActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmStoreActionPerformed
  TriggerErrorsPanel errors = (TriggerErrorsPanel)SwingUtil.getTabbedPaneComponent(TriggerErrorsPanel.class, this);
  syntaxSource.getStatusBar().getPanel("ddl-status").setText(stringManager.getString("executing-3d"));
  try {
    syntaxSource.storeObject();
    if (errors != null) {
      errors.refresh();
      TriggersPanelView view = (TriggersPanelView)SwingUtil.getOwnerComponent(TriggersPanelView.class, this);
      if (view != null) {
        view.refresh();
      }
      ...
      if (!errors.getQuery().eof()) {
        ...
        if (errorSettings.getValue(OracleCompileErrorSettingsProvider.setOnErrorGoToTab, true)) {
          ...
          gotoPoint(            
            errors.getQuery().fieldByName("line").getInteger() +StringUtil.nlineCount(errors.getQuery().fieldByName("description").getString()),
