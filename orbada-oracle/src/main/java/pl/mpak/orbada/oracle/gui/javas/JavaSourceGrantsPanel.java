 ...
 * @author  akaluza
 */
public class JavaSourceGrantsPanel extends javax.swing.JPanel implements ITabObjectInfo {
   ...
   */
  public JavaSourceGrantsPanel(IViewAccesibilities accesibilities) {
  ...
  private void init() {
    ...
    try {
      ...
      filter = new SqlFilter(
        accesibilities.getApplication().getSettings(getDatabase().getUserProperties().getProperty("schemaId"), "oracle-java-source-grants-filter"),
        cmFilter, buttonFilter, 
        def);
    } catch (Exception ex) {
      ExceptionUtil.processException(ex);
    }
    new ComponentActionsAction(getDatabase(), tableGrants, buttonActions, menuActions, "oracle-java-grants-actions");
  }
  ...
  public String getTitle() {
    return stringManager.getString("JavaSourceGrantsPanel-title");
  }
  ...
  public void refresh() {
    try {
      ...
      tableGrants.getQuery().paramByName("table_name").setString(currentObjectName);
...
private void cmGrantsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmGrantsActionPerformed
  if (SqlCodeWizardDialog.show(new GrantFunctionPrivilegesWizard(getDatabase(), currentSchemaName, currentObjectName), true) != null) {
    refresh();
  }
...
private void cmRevokeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmRevokeActionPerformed
  if (SqlCodeWizardDialog.show(new RevokeFunctionPrivilegesWizard(getDatabase(), currentSchemaName, currentObjectName), true) != null) {
    refresh();
  }
