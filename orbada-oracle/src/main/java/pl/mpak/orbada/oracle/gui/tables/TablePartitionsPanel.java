 ...
 * @author  akaluza
 */
public class TablePartitionsPanel extends javax.swing.JPanel implements ITabObjectInfo {
   ...
   */
  public TablePartitionsPanel(IViewAccesibilities accesibilities) {
  ...
  private void init() {
    ...
    try {
      ...
      filter = new SqlFilter(
        accesibilities.getApplication().getSettings(getDatabase().getUserProperties().getProperty("schemaId"), "oracle-table-partitions-filter"),
        cmFilter, buttonFilter, 
        def);
    } catch (Exception ex) {
      ExceptionUtil.processException(ex);
    }
    new ComponentActionsAction(getDatabase(), tablePartitions, buttonActions, menuActions, "oracle-table-partitions-actions");
  }
  ...
  public String getTitle() {
    return stringManager.getString("TablePartitionsPanel-title");
  }
  ...
  public void refresh() {
    try {
      ...
      tablePartitions.getQuery().setSqlText(Sql.getTablePartitionList(filter.getSqlText()));
      tablePartitions.getQuery().paramByName("schema_name").setString(currentSchemaName);
      tablePartitions.getQuery().paramByName("table_name").setString(currentTableName);
