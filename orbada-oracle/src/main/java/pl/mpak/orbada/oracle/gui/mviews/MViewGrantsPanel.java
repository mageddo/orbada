...
import pl.mpak.orbada.oracle.gui.wizards.mview.GrantMViewPrivilegesWizard;
import pl.mpak.orbada.oracle.gui.wizards.mview.RevokeMViewPrivilegesWizard;
 ...
 * @author  akaluza
 */
public class MViewGrantsPanel extends javax.swing.JPanel implements ITabObjectInfo {
  ...
  public MViewGrantsPanel(IViewAccesibilities accesibilities) {
  ...
  private void init() {
    ...
    try {
      ...
      filter = new SqlFilter(
        accesibilities.getApplication().getSettings(getDatabase().getUserProperties().getProperty("schemaId"), "oracle-mview-grants-filter"),
        cmFilter, buttonFilter, 
        def);
    } catch (Exception ex) {
      ExceptionUtil.processException(ex);
    }
    new ComponentActionsAction(getDatabase(), tableGrants, buttonActions, menuActions, "oracle-mview-grants-actions");
  }
  ...
  public String getTitle() {
    return stringManager.getString("MViewGrantsPanel-title");
  }
...
private void cmGrantsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmGrantsActionPerformed
  if (SqlCodeWizardDialog.show(new GrantMViewPrivilegesWizard(getDatabase(), currentSchemaName, currentViewName), true) != null) {
    refresh();
  }
...
private void cmRevokeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmRevokeActionPerformed
  if (SqlCodeWizardDialog.show(new RevokeMViewPrivilegesWizard(getDatabase(), currentSchemaName, currentViewName), true) != null) {
    refresh();
  }
