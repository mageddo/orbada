...
import pl.mpak.orbada.oracle.gui.wizards.view.GrantViewPrivilegesWizard;
import pl.mpak.orbada.oracle.gui.wizards.view.RevokeViewPrivilegesWizard;
...
public class ViewGrantsPanel extends javax.swing.JPanel implements ITabObjectInfo {
  ...
  private void init() {
    ...
    try {
      ...
      filter = new SqlFilter(
        accesibilities.getApplication().getSettings(getDatabase().getUserProperties().getProperty("schemaId"), "oracle-view-grants-filter"),
        cmFilter, buttonFilter, 
        def);
    } catch (Exception ex) {
      ExceptionUtil.processException(ex);
    }
    new ComponentActionsAction(getDatabase(), tableGrants, buttonActions, menuActions, "oracle-view-grants-actions");
  }
  ...
  public void refresh() {
    try {
      ...
      tableGrants.getQuery().paramByName("table_name").setString(currentViewName);
  ...
  private void initComponents() {
    ...
    cmGrants.setActionCommandKey("cmGrants");
...
private void cmGrantsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmGrantsActionPerformed
  if (SqlCodeWizardDialog.show(new GrantViewPrivilegesWizard(getDatabase(), currentSchemaName, currentViewName), true) != null) {
    refresh();
  }
...
private void cmRevokeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmRevokeActionPerformed
  if (SqlCodeWizardDialog.show(new RevokeViewPrivilegesWizard(getDatabase(), currentSchemaName, currentViewName), true) != null) {
    refresh();
  }
