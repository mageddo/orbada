...
public class FindObjectFromEditorAction extends Action {
  ...
  private boolean resolveObject(String type, String schema, String object) {
    if (StringUtil.anyOfString(type, new String[] {"TABLE", "VIEW", "MATERIALIZED VIEW", "PROCEDURE", "FUNCTION", "PACKAGE", "TYPE", "JAVA CLASS"}, true) != -1) {
    ...
    return new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        try {
          ...
          String object = null;
          String schema = null; //OracleDbInfoProvider.getCurrentSchema(database);
          if (words != null && words.length > 0) {
            ...
            try {
              ...
              if (!found) {
                object = schema;
                schema = null; //OracleDbInfoProvider.getCurrentSchema(database);
                query.close();
                query.paramByName("schema_name").setString(schema);
                query.paramByName("object_name").setString(object);
                query.open();
