 ...
 * @author  akaluza
 */
public class TypeGrantsPanel extends javax.swing.JPanel implements ITabObjectInfo {
   ...
   */
  public TypeGrantsPanel(IViewAccesibilities accesibilities) {
  ...
  private void init() {
    ...
    try {
      ...
      filter = new SqlFilter(
        accesibilities.getApplication().getSettings(getDatabase().getUserProperties().getProperty("schemaId"), "oracle-type-grants-filter"),
        cmFilter, buttonFilter, 
        def);
    } catch (Exception ex) {
      ExceptionUtil.processException(ex);
    }
    new ComponentActionsAction(getDatabase(), tableGrants, buttonActions, menuActions, "oracle-type-grants-actions");
  }
  ...
  public String getTitle() {
    return stringManager.getString("TypeGrantsPanel-title");
  }
  ...
  public void refresh() {
    try {
      ...
      tableGrants.getQuery().paramByName("table_name").setString(currentTypeName);
...
private void cmGrantsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmGrantsActionPerformed
  if (SqlCodeWizardDialog.show(new GrantTypePrivilegesWizard(getDatabase(), currentSchemaName, currentTypeName), true) != null) {
    refresh();
  }
...
private void cmRevokeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmRevokeActionPerformed
  if (SqlCodeWizardDialog.show(new RevokeTypePrivilegesWizard(getDatabase(), currentSchemaName, currentTypeName), true) != null) {
    refresh();
  }
