...
import pl.mpak.orbada.oracle.gui.wizards.GrantProcedurePrivilegesWizard;
import pl.mpak.orbada.oracle.gui.wizards.RevokeProcedurePrivilegesWizard;
...
public class ProcedureGrantsPanel extends javax.swing.JPanel implements ITabObjectInfo {
  ...
  private void init() {
    ...
    try {
      ...
      filter = new SqlFilter(
        accesibilities.getApplication().getSettings(getDatabase().getUserProperties().getProperty("schemaId"), "oracle-procedure-grants-filter"),
        cmFilter, buttonFilter, 
        def);
    } catch (Exception ex) {
      ExceptionUtil.processException(ex);
    }
    new ComponentActionsAction(getDatabase(), tableGrants, buttonActions, menuActions, "oracle-procedure-grants-actions");
  }
  ...
  public String getTitle() {
    return stringManager.getString("ProcedureGrantsPanel-title");
  }
...
private void cmGrantsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmGrantsActionPerformed
  if (SqlCodeWizardDialog.show(new GrantProcedurePrivilegesWizard(getDatabase(), currentSchemaName, currentObjectName), true) != null) {
    refresh();
  }
...
private void cmRevokeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmRevokeActionPerformed
  if (SqlCodeWizardDialog.show(new RevokeProcedurePrivilegesWizard(getDatabase(), currentSchemaName, currentObjectName), true) != null) {
    refresh();
  }
