...
public class TableTriggersPanel extends javax.swing.JPanel implements ITabObjectInfo {
  ...
  private void init() {
    settings = accesibilities.getApplication().getSettings(getDatabase().getUserProperties().getProperty("schemaId"), "oracle-table-triggers-panel");
    ...
    textTrigger.setStructureParser(new OraclePlSqlStructureParser());
    ...
    try {
      ...
      filter = new SqlFilter(
        accesibilities.getApplication().getSettings(getDatabase().getUserProperties().getProperty("schemaId"), "oracle-table-triggers-filter"),
        cmFilter, buttonFilter, 
        def);
    } catch (Exception ex) {
      ExceptionUtil.processException(ex);
    }
    new ComponentActionsAction(getDatabase(), tableTriggers, buttonActions, menuActions, "oracle-table-triggers-actions");
  }
  ...
  private void initComponents() {
    ...
    menuEnableAll = new javax.swing.JMenuItem();
    ...
    cmEnableAllTriggers = new pl.mpak.sky.gui.swing.Action();
    ...
    menuEnableAll.setAction(cmEnableAllTriggers);
    menuActions.add(menuEnableAll);
    ...
    cmEnableAllTriggers.setActionCommandKey("cmEnableAllTriggers");
    cmEnableAllTriggers.setSmallIcon(pl.mpak.sky.gui.swing.ImageManager.getImage("/pl/mpak/res/icons/enable_all.gif")); // NOI18N
    cmEnableAllTriggers.setText(stringManager.getString("cmEnableAllTriggers-text")); // NOI18N
    cmEnableAllTriggers.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(java.awt.event.ActionEvent evt) {
        cmEnableAllTriggersActionPerformed(evt);
      }
    });
    ...
    toolButton3.setAction(cmFreezeObject);
    ...
    toolButton1.setAction(cmEnableTrigger);
    ...
    toolButton4.setAction(cmEnableAllTriggers);
    ...
    toolButton5.setAction(cmDropTrigger);
    ...
    toolBarTriggers.add(toolButton5);
    toolBarTriggers.add(jSeparator7);
  ...
  if (tableTriggers.getSelectedRow() >= 0) {
    try {
      ...
      if (SqlCodeWizardDialog.show(new DropTableTriggerWizard(getDatabase(), currentSchemaName, currentTableName, triggerName), true) != null) {
        refresh();
      }
    } catch (Exception ex) {
...
private void cmCreatePKColumnTriggerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmCreatePKColumnTriggerActionPerformed
  try {
    if (SqlCodeWizardDialog.show(new CreateTriggerPKColumnWizard(getDatabase(), currentSchemaName, currentTableName), true) != null) {
      refresh();
    }
  } catch (Exception ex) {
...
private void cmEnableTriggerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmEnableTriggerActionPerformed
  if (tableTriggers.getSelectedRow() >= 0) {
    try {
      tableTriggers.getQuery().getRecord(tableTriggers.getSelectedRow());
      String triggerName = tableTriggers.getQuery().fieldByName("trigger_name").getString();
      if (SqlCodeWizardDialog.show(new EnableTriggerWizard(getDatabase(), currentSchemaName, currentTableName, triggerName), true) != null) {
        refresh();
      }
    } catch (Exception ex) {
      MessageBox.show(stringManager.getString("error"), ex.getMessage(), ModalResult.OK);
...
private void cmEnableAllTriggersActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmEnableAllTriggersActionPerformed
  if (SqlCodeWizardDialog.show(new EnableAllTableTriggersWizard(getDatabase(), currentSchemaName, currentTableName), true) != null) {
    refresh();
  }
}//GEN-LAST:event_cmEnableAllTriggersActionPerformed
...
private void cmCreateTriggerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmCreateTriggerActionPerformed
  if (SqlCodeWizardDialog.show(new CreateTableTriggerWizard(getDatabase(), currentSchemaName, currentTableName), true) != null) {
    refresh();
  }
...
private void cmCompileActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmCompileActionPerformed
  ...
  private pl.mpak.sky.gui.swing.Action cmEnableAllTriggers;
  ...
  private javax.swing.JMenuItem menuEnableAll;
