...
import pl.mpak.orbada.oracle.gui.wizards.GrantPackagePrivilegesWizard;
import pl.mpak.orbada.oracle.gui.wizards.RevokePackagePrivilegesWizard;
 ...
 * @author  akaluza
 */
public class PackageGrantsPanel extends javax.swing.JPanel implements ITabObjectInfo {
   ...
   */
  public PackageGrantsPanel(IViewAccesibilities accesibilities) {
  ...
  private void init() {
    ...
    try {
      ...
      filter = new SqlFilter(
        accesibilities.getApplication().getSettings(getDatabase().getUserProperties().getProperty("schemaId"), "oracle-package-grants-filter"),
        cmFilter, buttonFilter, 
        def);
    } catch (Exception ex) {
      ExceptionUtil.processException(ex);
    }
    new ComponentActionsAction(getDatabase(), tableGrants, buttonActions, menuActions, "oracle-package-grants-actions");
  }
  ...
  public String getTitle() {
    return stringManager.getString("PackageGrantsPanel-title");
  }
  ...
  public void refresh() {
    try {
      ...
      tableGrants.getQuery().paramByName("table_name").setString(currentPackageName);
...
private void cmGrantsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmGrantsActionPerformed
  if (SqlCodeWizardDialog.show(new GrantPackagePrivilegesWizard(getDatabase(), currentSchemaName, currentPackageName), true) != null) {
    refresh();
  }
...
private void cmRevokeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmRevokeActionPerformed
  if (SqlCodeWizardDialog.show(new RevokePackagePrivilegesWizard(getDatabase(), currentSchemaName, currentPackageName), true) != null) {
    refresh();
  }
