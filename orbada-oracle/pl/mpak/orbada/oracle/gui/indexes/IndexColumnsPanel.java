 ...
 * @author  akaluza
 */
public class IndexColumnsPanel extends javax.swing.JPanel implements ITabObjectInfo {
   ...
   */
  public IndexColumnsPanel(IViewAccesibilities accesibilities) {
  ...
  private void init() {
    ...
    try {
      tableColumns.addColumn(new QueryTableColumn("column_position", stringManager.getString("pos"), 30));
      ...
      tableColumns.addColumn(new QueryTableColumn("column_expression", stringManager.getString("expression"), 100));
      tableColumns.addColumn(new QueryTableColumn("descend", stringManager.getString("descend-q"), 50));
      SqlFilterDef def = new SqlFilterDef();
      def.add(new SqlFilterDefComponent("uic.column_name", stringManager.getString("column-name"), (String[])null));
      filter = new SqlFilter(
        accesibilities.getApplication().getSettings(getDatabase().getUserProperties().getProperty("schemaId"), "oracle-index-columns-filter"),
        cmFilter, buttonFilter, 
        def);
    } catch (Exception ex) {
      ExceptionUtil.processException(ex);
    }
    new ComponentActionsAction(getDatabase(), tableColumns, buttonActions, menuActions, "oracle-index-columns-actions");
  }
  ...
  public String getTitle() {
    return stringManager.getString("IndexColumnsPanel-title");
  }
  ...
  public void refresh() {
    try {
      ...
      tableColumns.getQuery().setSqlText(Sql.getIndexColumnList(filter.getSqlText()));
      tableColumns.getQuery().paramByName("schema_name").setString(currentSchemaName);
      tableColumns.getQuery().paramByName("index_name").setString(currentIndexName);
