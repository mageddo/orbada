 ...
 * @author  akaluza
 */
public class JavaSourceErrorsPanel extends javax.swing.JPanel implements ITabObjectInfo {
   ...
   */
  public JavaSourceErrorsPanel(IViewAccesibilities accesibilities, String objectType) {
  ...
  private void init() {
    ...
    new ComponentActionsAction(getDatabase(), tableErrors, buttonActions, menuActions, "oracle-java-source-errors-actions");
  }
  ...
  public String getTitle() {
    return stringManager.getString("JavaSourceErrorsPanel-title");
  }
  ...
  public void refresh() {
    try {
      ...
      tableErrors.getQuery().paramByName("object_name").setString(currentObjectName);
...
private void cmGotoSourceActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmGotoSourceActionPerformed
  if (tableErrors.getSelectedRow() >= 0) {
    try {
      ...
      JavaSourcePanel source = (JavaSourcePanel)SwingUtil.getTabbedPaneComponent(JavaSourcePanel.class, this);
      if (source != null) {
        JTabbedPane tp = (JTabbedPane)SwingUtil.getOwnerComponent(JTabbedPane.class, this);
        if (tp != null) {
          tp.setSelectedComponent(source);
          String text = tableErrors.getQuery().fieldByName("text").getString();
          if (text.startsWith(currentObjectName +":")) {
            text = text.substring((currentObjectName +":").length());
            if (text.indexOf(':') > 0) {
              text = text.substring(0, text.indexOf(':'));
              source.gotoPoint(Integer.parseInt(text) +1, 1);
            }
          }
          else if (tableErrors.getQuery().fieldByName("line").getInteger() > 0) {
            source.gotoPoint(
