...
public class TableReferencedToPanel extends javax.swing.JPanel implements ITabObjectInfo {
  ...
  private void init() {
    ...
    try {
      tableReferencedTo.addColumn(new QueryTableColumn("constraint_name", stringManager.getString("foreign-key"), 150, new QueryTableCellRenderer(java.awt.Font.BOLD)));
      tableReferencedTo.addColumn(new QueryTableColumn("column_name", stringManager.getString("column-name"), 120));
      tableReferencedTo.addColumn(new QueryTableColumn("r_schema_name", stringManager.getString("schema"), 120));
      tableReferencedTo.addColumn(new QueryTableColumn("r_table_name", stringManager.getString("table-name"), 120));
      tableReferencedTo.addColumn(new QueryTableColumn("r_column_name", stringManager.getString("column-name"), 120));
      tableReferencedTo.addColumn(new QueryTableColumn("r_constraint_name", stringManager.getString("constraint-name"), 150));
      tableReferencedTo.addColumn(new QueryTableColumn("status", stringManager.getString("status"), 70));
    ...
    new ComponentActionsAction(getDatabase(), tableReferencedTo, buttonActions, menuActions, "oracle-table-referenced-to-actions");
  }
  ...
  public void refresh() {
    try {
      ...
      tableReferencedTo.getQuery().paramByName("schema_name").setString(currentSchemaName);
  ...
  private void initComponents() {
    ...
    tableReferencedTo = new pl.mpak.orbada.gui.comps.table.ViewTable();
...
private void cmRefreshActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmRefreshActionPerformed
  ...
  private pl.mpak.orbada.gui.comps.table.ViewTable tableReferencedTo;
