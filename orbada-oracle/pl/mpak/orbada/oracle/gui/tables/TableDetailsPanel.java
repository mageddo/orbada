 ...
 * @author  akaluza
 */
public class TableDetailsPanel extends javax.swing.JPanel implements ITabObjectInfo {
  ...
  private VerticalQueryTablePanel tableInfoPanel;
  ...
  private void init() {
    ...
    new ComponentActionsAction(getDatabase(), tableInfoPanel.getTable(), buttonActions, menuActions, "oracle-table-details-actions");
  }
  ...
  public void refresh() {
    try {
      ...
      try {
        query.setSqlText(Sql.getTableTypeInfo());
        query.paramByName("SCHEMA_NAME").setString(currentSchemaName);
        query.paramByName("TABLE_NAME").setString(currentTableName);
        query.open();
        if (!query.eof()) {
          tableType = query.fieldByName("table_type").getString();
        }
      }
      catch (Exception ex) {
      ...
      if ("TABLE".equals(tableType)) {
        tableInfoPanel.refresh(Sql.getTableInfo(currentSchemaName, currentTableName));
      }
      else if ("OBJECT".equals(tableType)) {
        tableInfoPanel.refresh(Sql.getObjectTableInfo(currentSchemaName, currentTableName));
      }
      else if ("EXTERNAL".equals(tableType)) {
        tableInfoPanel.refresh(Sql.getExternalTableInfo(currentSchemaName, currentTableName));
      }
    } catch (Exception ex) {
