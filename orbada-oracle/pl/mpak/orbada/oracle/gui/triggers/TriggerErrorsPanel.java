 ...
 * @author  akaluza
 */
public class TriggerErrorsPanel extends javax.swing.JPanel implements ITabObjectInfo {
   ...
   */
  public TriggerErrorsPanel(IViewAccesibilities accesibilities) {
  ...
  private void init() {
    tableErrors.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
      public void valueChanged(ListSelectionEvent e) {
        int rowIndex = tableErrors.getSelectedRow();
        if (rowIndex >= 0 && tableErrors.getQuery().isActive()) {
          try {
            tableErrors.getQuery().getRecord(rowIndex);
          } catch (Exception ex) {
    ...
    new ComponentActionsAction(getDatabase(), tableErrors, buttonActions, menuActions, "oracle-trigger-errors-actions");
  }
  ...
  public String getTitle() {
    return stringManager.getString("TriggerErrorsPanel-title");
  }
  ...
  public void refresh() {
    try {
      ...
      tableErrors.getQuery().setSqlText(Sql.getTriggerErrorList());
      ...
      tableErrors.getQuery().paramByName("object_type").setString("TRIGGER");
...
private void cmGotoSourceActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmGotoSourceActionPerformed
  if (tableErrors.getSelectedRow() >= 0) {
    try {
      ...
      TriggerSourcePanel source = (TriggerSourcePanel)SwingUtil.getTabbedPaneComponent(TriggerSourcePanel.class, this);
      if (source != null) {
        JTabbedPane tp = (JTabbedPane)SwingUtil.getOwnerComponent(JTabbedPane.class, this);
        if (tp != null) {
          tp.setSelectedComponent(source);
          source.gotoPoint(
            tableErrors.getQuery().fieldByName("line").getInteger() +StringUtil.nlineCount(tableErrors.getQuery().fieldByName("description").getString()),
