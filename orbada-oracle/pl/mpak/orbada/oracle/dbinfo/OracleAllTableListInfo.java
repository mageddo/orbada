 ...
 * @author akaluza
 */
public class OracleAllTableListInfo extends DbObjectContainer<OracleAllTableInfo> {
  
  public OracleAllTableListInfo(OracleSchemaInfo owner) {
    super("ALL TABLES", owner);
  }
  ...
  public void refresh() throws Exception {
    ...
    try {
      ...
      try {
        query.setSqlText(Sql.getAllTableList(getFilter()));
        query.paramByName("schema_name").setString(getSchema().getName());
        query.open();
        while (!query.eof()) {
          OracleAllTableInfo info = new OracleAllTableInfo(query.fieldByName("table_name").getString(), this);
