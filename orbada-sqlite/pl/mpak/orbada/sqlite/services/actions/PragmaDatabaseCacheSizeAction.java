 ...
 * @author akaluza
 */
public class PragmaDatabaseCacheSizeAction extends ComponentActionProvider {

  private final StringManager stringManager = StringManagerFactory.getStringManager(OrbadaSQLitePlugin.class);

  public PragmaDatabaseCacheSizeAction() {
    super();
  }

  public PragmaDatabaseCacheSizeAction(Database database) {
    super();
        ...
        if (getComponent() instanceof ViewTable) {
          ...
          if (vt.getSelectedRow() >= 0) {
            try {
              ...
              SqlCodeWizardDialog.WizardResult result = SqlCodeWizardDialog.show(new PragmaCacheSizeWizard(getDatabase(), vt.getQuery().fieldByName("name").getString()), true);
  ...
  private void setProperties() {
    ...
    setActionCommandKey("PragmaDatabaseCacheSizeAction");
