package pl.mpak.sky.gui.swing.vtab;
...
public class VTextIcon implements Icon, PropertyChangeListener {
  ...
  void calcDimensions() {
    ...
    if (fRotation == ROTATE_NONE) {
      ...
      // and height is the font height * the char count, + one extra leading at
      // the bottom
  ...
  public void paintIcon(Component c, Graphics g, int x, int y) {
    ...
    if (fRotation == ROTATE_NONE) {
      ...
      for (int i = 0; i < fCharStrings.length; i++) {
        // Special rules for Japanese - "half-height" characters (like ya, yu,
        // yo in combinations)
        ...
        switch (fPosition[i]) {
          case POSITION_NORMAL:
            ...
            // monospaced.
            g.drawString(fCharStrings[i], x + ((fWidth - fCharWidths[i]) / 2),
                yPos);
            break;
   ...
   * text using the Arabic script in vertical lines, it is more common to employ
   * a horizontal baseline that is rotated by 90¡ counterclockwise so that the
   ...
   * top-to-bottom and are arrange columns either from left-to-right (Mongolian)
   * or right-to-left (other scripts). Most characters use the same shape and
