 ...
 * @author akaluza
 */
public class CreateForeignKeyColumnConstraintAction extends ComponentAction {

  private final StringManager stringManager = StringManagerFactory.getStringManager(OrbadaMySQLPlugin.class);

  public CreateForeignKeyColumnConstraintAction() {
    super();
        ...
        if (getComponent() instanceof ViewTable) {
          ...
          if (vt.getSelectedRow() >= 0) {
            try {
              ...
              if (SqlCodeWizardDialog.show(new CreateForeignKeyTableConstraintWizardPanel(getDatabase(), databaseName, tableName, columnName), true) != null) {
                ip.refresh();
              }
            } catch (Exception ex) {
