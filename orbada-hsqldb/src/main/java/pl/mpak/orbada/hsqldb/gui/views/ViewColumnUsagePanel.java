 ...
 * @author  akaluza
 */
public class ViewColumnUsagePanel extends javax.swing.JPanel implements ITabObjectInfo {
   ...
   */
  public ViewColumnUsagePanel(IViewAccesibilities accesibilities) {
  ...
  private void init() {
    ...
    try {
      tableColumns.addColumn(new QueryTableColumn("table_schema", stringManager.getString("schema"), 150));
      tableColumns.addColumn(new QueryTableColumn("table_name", stringManager.getString("table-name"), 150));
      tableColumns.addColumn(new QueryTableColumn("column_name", stringManager.getString("column-name"), 150));
      SqlFilterDef def = new SqlFilterDef();
      def.add(new SqlFilterDefComponent("table_schema", stringManager.getString("schema"), (String[])null));
      ...
      filter = new SqlFilter(
        accesibilities.getApplication().getSettings(getDatabase().getUserProperties().getProperty("schemaId"), "hsqldb-view-column-usage-filter"),
        cmFilter, buttonFilter, 
        def);
    } catch (Exception ex) {
      ExceptionUtil.processException(ex);
    }
    new ComponentActionsAction(getDatabase(), tableColumns, buttonActions, menuActions, "hsqldb-view-column-usages-actions");
  }
  ...
  public String getTitle() {
    return stringManager.getString("ViewColumnUsagePanel-title");
  }
  ...
  public void refresh() {
    try {
      ...
      tableColumns.getQuery().setSqlText(Sql.getViewColumnUsageList(filter.getSqlText(), HSqlDbInfoProvider.getVersionTest(getDatabase())));
      tableColumns.getQuery().paramByName("schema_name").setString(currentSchemaName);
      tableColumns.getQuery().paramByName("view_name").setString(currentTableName);
